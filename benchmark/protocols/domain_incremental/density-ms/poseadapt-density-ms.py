...
def make_dataset(split, level, test_mode=False):
    ...
    if level == "coco":
        ...
        ann_file = f"person_keypoints_{split}2017-{pa_type}-{level}.json"
...
dataset_train3 = make_multi_dataset(pa_levels[2:3])
dataset_train4 = make_multi_dataset(pa_levels[3:4])
...
dataset_val3 = make_multi_dataset(pa_levels[:3], is_val=True)
dataset_val4 = make_multi_dataset(pa_levels[:4], is_val=True)
    ...
    lvl: (
        ...
        if lvl == "coco"
            ...
            "ann_file": osp.join(
                posebench_root,
                "annotations",
                f"person_keypoints_val2017-{pa_type}-{lvl}.json",
            ),
            "prefix": lvl,
        }
    )
...
evaluator_val3 = make_evaluator(pa_levels[:3])
evaluator_val4 = make_evaluator(pa_levels[:4])
